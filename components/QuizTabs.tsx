  ...
  return (
    ...
    >
      <div className="mx-auto max-w-3xl px-4 py-3 sm:px-6 lg:px-8">
        <div className="inline-flex items-center gap-1.5 rounded-full border border-white/8 bg-white/[0.03] p-1.5">
          {tabs.map((tab) => {
            ...
            return (
              <Link
                key={tab.id}
                href={tab.href}
                className={`relative flex items-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition-all duration-150 ${isActive ? "nav-link-active bg-white/[0.06] text-white" : "text-white/42 hover:bg-white/[0.05] hover:text-white/88"}`}
              >
                <span>{tab.icon}</span>
                <span>{tab.label}</span>
              </Link>
